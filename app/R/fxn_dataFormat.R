...
fxn_dataFormat <- function(inData, timeStep) {
  ...
  if (timeStep == "Hourly") {
    dataFormat <- inData %>%
      dplyr::mutate(
        dplyr::across(
          c(
            "date_doy", "date_year", "meta_needs_review", "meta_version", "relative_humidity", "wind_2min_vector_dir", "wind_vector_dir", "wind_vector_dir_stand_dev"
          ),
          \(x) format(x, nsmall = 0)
        )
      ) %>%
      ...
      dplyr::mutate(
        dplyr::across(
          c(
            "eto_azmet_in", "meta_bat_volt", "precip_total_in", "sol_rad_total", "sol_rad_total_ly", "vp_actual", "vp_deficit"
          ), 
          \(x) format(x, nsmall = 2)
        )
      ) %>%
  ...
  if (timeStep == "Daily") {
    dataFormat <- inData %>%
      dplyr::mutate(
        dplyr::across(
          c(
            "chill_hours_0C", "chill_hours_20C", "chill_hours_32F", "chill_hours_45F", "chill_hours_68F", "chill_hours_7C", "date_doy", "date_year", "meta_needs_review", "meta_version", "relative_humidity_max", "relative_humidity_mean", "relative_humidity_min", "wind_2min_vector_dir", "wind_vector_dir", "wind_vector_dir_stand_dev"
          ), 
          \(x) format(x, nsmall = 0)
        )
      ) %>%
      ...
      dplyr::mutate(
        dplyr::across(
          c(
            "eto_azmet_in", "eto_pen_mon_in", "meta_bat_volt_max", "meta_bat_volt_mean", "meta_bat_volt_min", "precip_total_in", "sol_rad_total", "sol_rad_total_ly", "vp_actual_max", "vp_actual_mean", "vp_actual_min", "vp_deficit_mean"
          ), 
          \(x) format(x, nsmall = 2)
        )
      ) %>%
      dplyr::mutate(dplyr::across(dplyr::everything(), as.character)) %>%
      dplyr::select(order(colnames(inData))) %>%
      dplyr::relocate(datetime, .before = dplyr::everything())
  }
