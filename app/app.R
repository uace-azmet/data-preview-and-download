  ...
  function(input, output, session) {
    ...
    shiny::observeEvent(input$azmetStation, {
      ...
      if (input$timeStep == "Daily") {
        stationDateMaximum <-
          dplyr::filter(azmetStationMetadata, meta_station_name == input$azmetStation) %>% 
          dplyr::pull(end_date_daily)
      } else if (input$timeStep == "Hourly") {
        stationDateMaximum <-
          dplyr::filter(azmetStationMetadata, meta_station_name == input$azmetStation) %>% 
          dplyr::pull(end_date_hourly)
      }
    ...
    dataETL <- 
      shiny::eventReactive(input$previewData, {
        shiny::validate(
          shiny::need(expr = input$startDate <= input$endDate, message = FALSE)
        )
